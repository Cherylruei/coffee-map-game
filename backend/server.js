...
app.get('/api/admin/stats', authenticateAdmin, (req, res) => {
    const stats = {
        ...
        totalQRCodes: Array.from(DB.qrCodes.values()).filter(q => q.type === 'gacha').length,
        usedQRCodes: Array.from(DB.qrCodes.values()).filter(q => q.type === 'gacha' && q.used).length,
        cardDistribution: {}
    };

    // 計算卡片分布
    for (let i = 1; i <= 12; i++) {
        stats.cardDistribution[i] = DB.gachaHistory.filter(h => h.cardId === i).length;
    }
