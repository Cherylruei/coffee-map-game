...
**Week 7 修復** — 對應本章末尾「記錄問題 1」，這次一併正式解掉。

---

### 問題是怎麼發現的

不是靠測試發現的，是靠**畫流程圖時發現的**：把「產生 QR → 顧客掃描 → 店員按完成」三個步驟畫出來後，發現中間有一段沒人把關——
...
顧客到底有沒有掃碼、有沒有真的扣到錢，**系統從頭到尾沒有問過**。等於只要店員手滑先按完成、或顧客根本沒打算付錢，一杯咖啡就這樣送出去了。
...
```
第一棒（產生 QR）    →    第二棒（掃碼扣款）    →    第三棒（完成訂單）
     ✅ 有做好              ⚠️ 顧客可能沒接棒            ✅ 有做好
                                                    但沒人檢查前面有沒有交棒成功！
```

技術上這是「Check-Then-Act」問題：系統做了「查詢 QR 狀態」跟「建立訂單」兩件事，但兩件事之間沒有任何強制關聯——沒人規定「一定要先確認查到 used=true，才准許執行下一步」。
...
| 層級 | 做法 | 檔案位置 |
|------|------|---------|
| **後端強制檢查**（最關鍵）| `payment_method === 'wallet'` 時，強制查 `qr_codes.used = true` 且 `wallet_amount` 與訂單金額相符，否則直接拒絕 | `server-supabase.js` `/api/admin/order` |
| **前端輪詢＋disable** | 開啟 QR 畫面後每 3 秒查一次使用狀態，顧客掃描確認前「完成收款」按鈕保持disabled，並顯示「⏳ 等待客人掃描確認扣款中…」 | `QRViewer.tsx` + `OrderTab.tsx`（新端點 `GET /api/admin/qrcode/status/:code`）|
| **原子性搶佔（compare-and-swap）** | 同一張已掃描的 QR 只能成功換到一次訂單，用 `UPDATE ... WHERE wallet_order_claimed_at IS NULL` 做搶佔，搶不到就拒絕 | `qr_codes.wallet_order_claimed_at` 新欄位（`database/add_qr_wallet_order_claim.sql`）|

**第三層是怎麼被抓出來的？** 這次沒有自己想到，是請 `security-reviewer` agent 覆核第一版修法時抓出來的：**光靠「檢查 used=true」是不夠的，因為檢查完之後狀態沒有被消耗掉，同一張已掃描成功的 QR 理論上可以反覆拿去建立好幾筆訂單**（例如網路重試、店員手滑按兩次、兩台後台裝置同時送出）。這跟「顧客沒掃碼」是完全不同的漏洞，但都源自同一個根因：**檢查（check）和使用（act）中間沒有鎖**。

```sql
-- 搶佔的核心邏輯（compare-and-swap）
UPDATE qr_codes
SET wallet_order_claimed_at = NOW()
WHERE code = $1
  AND wallet_order_claimed_at IS NULL
-- 回傳 0 筆 → 已經被搶走了 → 拒絕這次請求
-- 回傳 1 筆 → 搶到了 → 可以繼續建立訂單
```

這跟十三章「防護一：Race Condition」的 `SELECT ... FOR UPDATE` 是同一個家族的技巧，只是換了一種寫法（用 `WHERE 條件 IS NULL` 取代 `FOR UPDATE` 鎖），效果一樣：**兩個同時打進來的請求，只有一個能成功，Postgres 保證不會兩個都通過**。

---

### 新學到的知識點：Check-Then-Act / TOCTOU

**TOCTOU = Time-Of-Check to Time-Of-Use（檢查時間與使用時間的落差）**

這是一個資安領域的經典漏洞類別，不是這個專案獨有的：

```
時間點 A：檢查「這張票券還沒用過」→ 通過
        ↓（這中間如果有別的請求插進來...）
時間點 B：真的去用這張票券
```

只要「檢查」和「使用」不是同一個不可分割的動作，中間就有縫隙可以鑽。**修法不是加更多檢查，而是把「檢查」跟「使用（消耗）」合併成同一個原子操作**——這也是為什麼十三章的 `claim_order_qr`、`deduct_wallet`、`topup_wallet` 都要用資料庫鎖或 compare-and-swap，而不是「查一次、確認沒問題、再更新」這種分兩步的寫法。

---

### 之後遇到類似專案，系統性規劃 checklist

> 這次的教訓抽象化後，可以變成一個通用的檢查清單，往後只要專案有「憑證 / 票券 / QR / 折扣碼 / 優惠券」這種「產生後被消費一次」的東西，開發前就先問這四個問題：

```
□ 1. 誰是唯一真相來源（Source of Truth）？
     → 這次的教訓：不能讓「前端知道的狀態」跟「後端資料庫的真相」出現落差，
       所有判斷都要以資料庫當下的資料為準，前端狀態只是「顯示用」的快取。

□ 2. 「消費」這個動作是否為原子操作（Atomic）？
     → 用 SELECT...FOR UPDATE 或 UPDATE...WHERE 條件 IS NULL 的 compare-and-swap，
       確保「檢查」跟「標記已使用」是同一個不可分割的步驟，不能分兩步寫。

□ 3. 同一張憑證，是否可能被「合法地」重複觸發？
     → 網路重試、使用者連點兩下、多裝置/多分頁同時操作，這些都是「正常操作」
       而非攻擊，但一樣要防。不能只靠前端 disable 按鈕當唯一防線
       （前端的 disabled 只防「不小心」，防不了 curl/Postman 或 race condition）。

□ 4. 金額/數量是否由後端重新計算，而不是相信前端傳來的欄位？
     → 這次 finalAmount 是後端用 items + discount 重新算出來的，
       前端傳來的 totalAmount 只拿來記 log，不會被拿去當真正入帳金額。
       這是「Never Trust Client」原則的具體實踐。
```

**用一句話總結：** 只要看到「兩段式流程」（A 產生憑證 → B 之後才消費），第一個反應就該是「這兩步之間有沒有鎖？」，而不是等到出包才回頭補。

---

### 對照表：這次用到 vs 十三/十八章已有的技術

| 技術 | 十三/十八章已學過 | 這次新增/延伸 |
|------|-----------------|---------------|
| `SELECT ... FOR UPDATE` | ✅ RPC 內用過 | — |
| `UPDATE ... WHERE 條件 IS NULL`（CAS）| — | ✅ 新學到，效果同 FOR UPDATE 但更輕量，適合「一次性標記」場景 |
| 後端重算金額，不信任前端 | ✅ 已有原則 | 這次具體驗證：`finalAmount` 由 items+discount 算出，非前端傳入 |
| Atomic Transaction | ✅ RPC 層級 | 這次在一般 API handler 內也用了同樣精神（claim → insert → 失敗就回滾 claim）|
| TOCTOU / Check-Then-Act | — | ✅ 新學到的資安詞彙，是這整個漏洞的根本分類 |

---

*最後更新：2026-07-10*
*新增：二十（儲值金付款防呆漏洞修復案例、TOCTOU 觀念、系統性規劃 checklist）*

---

記錄問題:
1. ~~後台儲值完，用戶入帳已確認，後台該 qrcode 是否要直接跳出已被使用狀態?~~
   ✅ 已於 2026-07-10 修復（見二十章）：後台現在會輪詢 QR 狀態，顧客掃描確認前「完成收款」按鈕會 disabled 並顯示等待中提示；後端也新增強制檢查，即使前端被繞過，未掃描/金額不符也無法建立訂單。
2. 待辦：`database/add_qr_wallet_order_claim.sql` 這個 migration 還沒套用到正式環境 Supabase，套用前這次的「防止同一張 QR 建立多筆訂單」保護不會生效（後端程式碼已經在跑，但資料庫還沒有 `wallet_order_claimed_at` 欄位，這段程式碼呼叫時會報錯）。
3. 已知限制（暫不處理，僅記錄）：如果店員用大額折扣把訂單金額殺到 $0，儲值金付款可以完全不需要 QR/掃描就通過（因為 finalAmount > 0 才會觸發檢查）。目前影響有限（沒有金流真的移動），但如果之後要做「防止店員亂給折扣」的稽核，這是已知的繞過點。